...
public class TranxApprovalAct extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(superProfile !=null){
            superAdmin=superProfile.getSuperAdmin();
        }
        if(superAdmin !=null){
            txApprover=superAdmin.getUFirstName();
        }
        ...
        if(transactionGranting !=null){
            customer=transactionGranting.getCustomer();
            userProfile=transactionGranting.getProfile();
                        ...
                        if (transactionArrayList.get(i).getDate().equalsIgnoreCase(todayDate) && transactionArrayList.get(i).getTransactionApprover().equalsIgnoreCase(txApprover) && transactionArrayList.get(i).getCustomerId()==customerID) {
                            ...
                            if(fromType.equalsIgnoreCase("StandingOrderAcct")){
                                ...
                                if(sOBalance>0.00){
                                    ...
                                    standingOrderAcct.setAccountBalance(newBalance);
