...
public class TranxSimpleAdapter extends RecyclerView.Adapter<TranxSimpleAdapter.ViewHolder> implements ListAdapter {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        holder.payer.setText(MessageFormat.format("Payer: {0}-{1}-{2}", transaction.getTranxPayer()));
        holder.status.setText(MessageFormat.format("Status: {0}", customerDailyReport.getRecordStatus()));

        holder.txtTransactionID.setText(MessageFormat.format("{0} - {1}", transaction.getTranXType().toString(), transaction.getTransactionID()));
        if (transaction.getTranXType() == Transaction.TRANSACTION_TYPE.PAYMENT) {
            ...
            holder.txtTransactionID.setText(MessageFormat.format("To Payee: {0}", transaction.getTranxPayee()));
            holder.txtTransactionAmount.setTextColor(Color.RED);
        } else if (transaction.getTranXType() == Transaction.TRANSACTION_TYPE.MANUAL_WITHDRAWAL) {
            holder.imgTransactionIcon.setImageResource(R.drawable.transfer3);

            holder.txtTransactionID.setText(MessageFormat.format("From: {0} - To: {1}", transaction.getTranxSendingAcct(), transaction.getTranxDestAcct()));
