...
public class EmergencyReportAdapter extends RecyclerView.Adapter<EmergencyReportAdapter.RecyclerViewHolder> implements Filterable {
        ...
        Filter filter = new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                for (EmergencyReport dataNames : itemsListFilter) {
                    if (dataNames.getType().toLowerCase().trim().contains(searchText)) {
    ...
    public void onBindViewHolder(@NonNull @NotNull RecyclerViewHolder holder, int position) {
        ...
        holder.reportType.setText(MessageFormat.format("Type{0}", recyclerData.getType()));
        holder.timeStamp.setText(MessageFormat.format("Time Stamp{0}", String.valueOf(recyclerData.getReportTime())));
        holder.latLng.setText(MessageFormat.format("Location{0}", recyclerData.getLatLng()));
        holder.status.setText(MessageFormat.format("Status{0}", recyclerData.getStatus()));
    }
