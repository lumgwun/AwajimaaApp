...
public class Clustering {
    ...
    public  static Map<String,Collection<Integer>> sweepAlgoclustering(List<Address> address){
        ...
        for(Map.Entry<Double,Integer> entry : treeMap.entrySet()){
            ...
            sum = sum + WasteMgtMainAct.demandList.get(entry.getValue());
            Log.e(GeocodeConstants.TAG_ROUTE,"demand " + WasteMgtMainAct.demandList.get(entry.getValue())
                    + " ,sum " + sum);

            if( sum <= WasteMgtMainAct.vehicleCapacity){
                multimap.put(cluster,entry.getValue());
            }
            else{
                c++;
                cluster = "cluster" + c;
                sum = WasteMgtMainAct.demandList.get(entry.getValue());
                multimap.put(cluster,entry.getValue());
            }
        }
