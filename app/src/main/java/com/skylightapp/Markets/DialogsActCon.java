...
import com.skylightapp.MarketClasses.QBPushSubListenerImplCon;
...
import com.skylightapp.MarketInterfaces.FcmConsts;
...
public class DialogsActCon extends BaseActCon implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = DialogsActCon.class.getSimpleName();
    ...
    private DialogsAdapterCon dialogsAdapter;
    private DialogsListener dialogsListener = new DialogsActCon.DialogsListener(TAG);
    private QBChatDialogMessageListener allDialogsMessagesListener = new DialogsActCon.AllDialogsMessageListener();
    private SystemMessagesListener systemMessagesListener = new DialogsActCon.SystemMessagesListener();
    private QBSystemMessagesManager systemMessagesManager;
    private QBIncomingMessagesManager incomingMessagesManager;
    private long lastClickTime = 0L;

    private QBUser currentUser;
    private androidx.appcompat.view.ActionMode currentActionMode;
    private boolean hasMoreDialogs = true;
    private final Set<DialogJoinerAsyncTask> joinerTasksSet = new HashSet<>();

    public static void start(Context context) {
        Intent intent = new Intent(context, DialogsActCon.class);
    ...
    public static void start(Context context, String dialogID) {
        Intent intent = new Intent(context, DialogsActCon.class);
        intent.putExtra(ConstsInterface.EXTRA_CERTAIN_DIALOG_ID, dialogID);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
        context.startActivity(intent);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String dialogToOpen = getIntent().getStringExtra(ConstsInterface.EXTRA_CERTAIN_DIALOG_ID);
        if (!TextUtils.isEmpty(dialogToOpen)) {
            Log.d(TAG, "Auto Starting ChatActivity with dialog : " + dialogToOpen);
            ChatActCon.startForResult(DialogsActCon.this, REQUEST_DIALOG_ID_FOR_UPDATE, dialogToOpen, false);
        }
        setContentView(R.layout.act_dialogs_act_con);

        if (getChatHelper().getCurrentUser() != null) {
            currentUser = getChatHelper().getCurrentUser();
        } else {
            ToastUtilsCon.shortToast(DialogsActCon.this, "Something went wrong with application cache");
            finish();
        }
    ...
    public void onResumeFinished() {
        if (isCallServiceRunning(CallServiceConf.class)) {
            hideProgressDialog();
            Log.d(TAG, "CallService is running now");
            CallActivityCon.start(this);
            return;
        }
        ...
        switch (item.getItemId()) {
            case R.id.menu_add_chat:
                ...
                if (!isProcessingResultInProgress) {
                    ...
                    SelectUsersActCon.startForResult(this, REQUEST_SELECT_PEOPLE, null);
                }
                return true;
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case REQUEST_SELECT_PEOPLE:
                    ArrayList<QBUser> selectedUsers = (ArrayList<QBUser>) data.getSerializableExtra(SelectUsersActCon.EXTRA_QB_USERS);
                    String chatName = data.getStringExtra(SelectUsersActCon.EXTRA_CHAT_NAME);
                    showProgressDialog(R.string.create_chat);
                    createDialog(selectedUsers, chatName);
                    break;
                case REQUEST_DIALOG_ID_FOR_UPDATE:
                    if (data != null) {
                        String dialogID = data.getStringExtra(ChatActCon.EXTRA_DIALOG_ID);
    ...
    private void unsubscribeFromPushesAndLogout() {
        ...
        if (QBPushManager.getInstance().isSubscribedToPushes()) {
            QBPushManager.getInstance().addListener(new QBPushSubListenerImplCon() {
                @Override
            ...
            SubscribeService.unSubscribeFromPushes(DialogsActCon.this);
        } else {
            userLogout();
        }
    }
    ...
    private void initUi() {
        ...
        dialogsAdapter = new DialogsAdapterCon(this, dialogs);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (currentActionMode != null) {
                    ...
                    getChatHelper().loginToChat(currentUser, new QBEntityCallback<Void>() {
                        ...
                        public void onSuccess(Void aVoid, Bundle bundle) {
                            hideProgressDialog();
                            ChatActCon.startForResult(DialogsActCon.this, REQUEST_DIALOG_ID_FOR_UPDATE, selectedDialog.getDialogId(), false);
                        }

                        @Override
                        public void onError(QBResponseException e) {
                            hideProgressDialog();
                            ToastUtilsCon.shortToast(DialogsActCon.this, e.getMessage());
                        }
                    });
                }
            }
    ...
    private void createDialog(final List<com.quickblox.users.model.QBUser> selectedUsers, String chatName) {
        ...
        getChatHelper().createDialogWithSelectedUsers(selectedUsers, chatName,
                new QBEntityCallback<QBChatDialog>() {
                    ...
                    public void onSuccess(QBChatDialog dialog, Bundle args) {
                        ...
                        new DialogJoinerAsyncTask(DialogsActCon.this, dialogsToJoin).execute();
                        getQBDialogsHolder().addDialog(dialog);
                        ChatActCon.startForResult(DialogsActCon.this, REQUEST_DIALOG_ID_FOR_UPDATE, dialog.getDialogId(), true);
                        hideProgressDialog();
                    }
    ...
    private void loadDialogsFromQb(final boolean silentUpdate, final boolean clearDialogHolder) {
        ...
        getChatHelper().getDialogs(requestBuilder, new QBEntityCallback<ArrayList<QBChatDialog>>() {
            ...
            public void onSuccess(ArrayList<QBChatDialog> dialogs, Bundle bundle) {
                ...
                DialogJoinerAsyncTask joinerTask = new DialogJoinerAsyncTask(DialogsActCon.this, dialogs);
    ...
    private void initConnectionListener() {
        ...
        chatConnectionListener = new VerboseQbChatConListenerCon(DialogsActCon.this, rootView) {
            @Override
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
        ...
        switch (menuItem.getItemId()) {
            case R.id.menu_video_settings: {
                SettingsActCon.start(this, true);
                break;
            }
            case R.id.menu_audio_settings: {
                SettingsActCon.start(this, false);
                break;
            }
    ...
    private class DeleteActionModeCallback implements ActionMode.Callback, androidx.appcompat.view.ActionMode.Callback {
        ...
        private void openAlertDialog() {
            AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(DialogsActCon.this, R.style.AlertDialogStyle);
        ...
        private void deleteSelected() {
            ...
            for (QBChatDialog dialog : selectedDialogs) {
                if (dialog.getType().equals(QBDialogType.PUBLIC_GROUP)) {
                    ToastUtilsCon.shortToast(DialogsActCon.this, getString(R.string.dialogs_cannot_delete_chat, dialog.getName()));
        ...
        private void leaveGroupDialogs(final List<QBChatDialog> groupDialogsToDelete) {
            getChatHelper().leaveGroupDialogs(groupDialogsToDelete, new QBEntityCallback<List<QBChatDialog>>() {
                ...
                public void onError(QBResponseException e) {
                    ...
                    runOnUiThread(() -> ToastUtilsCon.longToast(DialogsActCon.this, R.string.dialogs_deletion_error));
                }
            });
        }
    ...
    private static class DialogJoinerAsyncTask extends BaseAsyncTaskCon<Void, Void, Void> {
        private WeakReference<DialogsActCon> activityRef;
        private ArrayList<QBChatDialog> dialogs;

        DialogJoinerAsyncTask(DialogsActCon dialogsActivity, ArrayList<QBChatDialog> dialogs) {
